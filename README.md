...
```ini
[project]
dependencies#0 = https://github.com/yes2games/yes2sdk-defold/archive/refs/tags/v1.5.0.zip
```

Then in Defold Editor: **Project > Fetch Libraries**.

> Pinning the URL with `tags/v1.5.0` keeps the resolved hash stable. Bump the tag when a newer release ships. **Don't use `/refs/heads/main.zip`** — branch archives are served less reliably by GitHub and `Fetch Libraries` fails intermittently.
